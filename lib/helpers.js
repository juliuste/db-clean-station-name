...
const removeLeadingAndTrailingRelicts = name => {
	const correctedLeading = name.replace(/^[^\p{L}("'’„]+(?=[\p{L}("'’„])/gui, '')
	const alsoCorrectedTrailing = correctedLeading.replace(/(?<=[\p{L})"'’“])[^\p{L})"'’“]+$/gui, '')
...
const replaceStreet = name => {
	...
	const alsoReplacedUnambiguousWithoutPeriod = replacedWithPeriod.replace(/(?<=(^|[^\p{L}])str)(?=($|[^\p{L}]))/gui, 'aße')
