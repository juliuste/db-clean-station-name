...
const _abzweigRegExp = /(?<=^|[^\p{L}])abzw\.?(?=$|[^\p{L}])/gui
const replaceAbzweig = name => name.replace(_abzweigRegExp, 'Abzweig')
const _beiRegExp = /(?<=^|[^\p{L}])b\.?(?=\s+\p{L}+)/gui
const replaceBei = name => name.replace(_beiRegExp, 'bei')
const _richtungRegExp = /(?<=^|[^\p{L}])Ri\.?(?=\s+\p{L}+)/gui
const replaceRichtung = name => name.replace(_richtungRegExp, 'Richtung')
const replaceLocationAbbreviation = ({ short, long }) => {
	const r = new RegExp(`\\(${short}\\.?\\)`, 'gi')
	return name => {
		return name.replace(r, `(${long})`)
	}
}
const replaceAbbreviatedWord = ({ short, long }) => {
	const r = new RegExp(`(?<=(^|[^\\p{L}])${escapeRegExp(short.slice(0, 1))})${escapeRegExp(short.slice(1))}(?=($|[^\\p{L}]))`, 'gui')
	return name => name.replace(r, long.slice(1))
}
const removeBracketWithAbbreviation = abbreviation => {
	const r = new RegExp(`\\(${escapeRegExp(abbreviation)}\\)`, 'gi')
	return name => name.replace(r, ' ')
}
const removeLeadingAbbreviation = abbreviation => {
	const r = new RegExp(`(?<=^)${escapeRegExp(abbreviation)}(?=[\\s]+)`, 'gi')
	return name => name.replace(r, ' ')
}
const _streetRegExp = /(?<=str)[.]?(?=($|[^\p{L}]))/gui
const replaceStreet = name => name.replace(_streetRegExp, 'aße ')
