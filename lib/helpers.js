...
const replaceAbbreviatedWord = (abbreviation, replacement) => name => name.replace(new RegExp(`(?<=(^|[^\\p{L}])${escapeRegExp(abbreviation.slice(0, 1))})${escapeRegExp(abbreviation.slice(1))}(?=($|[^\\p{L}]))`, 'gui'), replacement.slice(1))
const removeBracketWithAbbreviation = abbreviation => name => name.replace(new RegExp(`\\(${escapeRegExp(abbreviation)}\\)`, 'gi'), ' ')
const replaceStreet = name => {
	const replacedWithPeriod = name.replace(/(?<=str)\./gi, 'aße ')
	const alsoReplacedUnambiguousWithoutPeriod = replaceAbbreviatedWord('Str', 'Straße ')(replacedWithPeriod)
	return alsoReplacedUnambiguousWithoutPeriod
}
