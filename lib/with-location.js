...
const removeTrailingInBrackets = (name, locationCandidates) => {
	const extractedName = get(name.match(/(?<=\s\()[^(]+(?=\)$)/ui), '[0]')
	if (!extractedName || extractedName.length === 0) return { name, matchedLocationKeys: [] }

	const matchingCandidate = locationCandidates.find(({ alias }) => extractedName === alias)
	if (!matchingCandidate) return { name, matchedLocationKeys: [] }

	const shortenedName = name.slice(0, -(2 + matchingCandidate.alias.length + 1))
	const cleanedShortenedName = cleanStationName(shortenedName)

	if (!cleanedShortenedName) return { name, matchedLocationKeys: [] }
	return { name: cleanedShortenedName, matchedLocationKeys: [matchingCandidate.key] }
}
...
const cleanStationNameWithLocation = (name, location) => {
	...
	const withoutTrailingInBrackets = removeTrailingInBrackets(withoutTrailing.name, locationCandidates)
	const withoutLeadingDash = removeLeadingForChar('-')(withoutTrailingInBrackets.name, locationCandidates)
	const withoutLeadingSpace = removeLeadingForChar(' ')(withoutLeadingDash.name, locationCandidates)

	const matchedLocationIds = uniq(flatMap([withoutTrailing, withoutTrailingInBrackets, withoutLeadingDash, withoutLeadingSpace], item => item.matchedLocationKeys))
